...
module git-completion-utils {
  ...
  export def get-mergable-sources []: nothing -> list<record<value: string, description: string>> {
    let local = get-all-git-local-refs | each {|x| {value: $x.ref description: $'Branch, Local, ($x.obj) ($x.subject), (if ($x.upstream | is-not-empty) { $x.upstream } else { "no upstream" } )'} } | insert style { fg: "#0d50c5" attr: b }
    let remote = get-all-git-remote-refs | each {|x| {value: $x.ref description: $'Branch, Remote, ($x.obj) ($x.subject)'} } | insert style { fg: "#0d50c5" attr: i }
    $local | append $remote
  }
}
...
def "nu-complete git checkout" [context: string, position?:int] {
  ...
  let branches = get-mergable-sources
  let files = (get-checkoutable-files) | wrap value | insert description 'File' | insert style { fg: "#4c6129" }
  let commits = ^git rev-list -n 400 --remotes --oneline | lines | split column -n 2 ' ' value description | upsert description {|x| $'Commit, ($x.value) ($x.description)' } | insert style { fg: "#006442" }
  {
    options: {
        case_sensitive: false,
        completion_algorithm: prefix,
